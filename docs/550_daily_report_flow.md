...
`新着SES（スコア70+）の技術者・案件 + 期限切れ間近のSES契約` の合計件数。
...
### 2.4 👤 新着技術者（スコア70+ 直近24h）

技術者紹介メール（`engineer_mail_sources`）のうち、スコア 70 以上で **直近24h 以内** に到着したものを表示。

- 同一技術者・同一スキルセットの重複は `(name|skills)` でユニーク化
- 上位 5 件＋ユニーク件数を表示
- 各エントリに「詳細を開く →」リンク（`/engineer-mails?select={id}`）で当該メールに直接遷移

### 2.5 📨 新着案件（スコア70+ 直近24h）

案件メール（`project_mail_sources`）のうち、スコア 70 以上で 直近24h 以内に到着したものを表示。

- 同一案件・同一取引先の重複は `(title|customer_name)` でユニーク化
- 上位 5 件＋ユニーク件数を表示
- 各エントリに「詳細を開く →」リンク（`/project-mails?select={id}`）で当該メールに直接遷移
...
SendDailySalesReport::handle()
  ...
  │   ├─ [1] 受信メール件数集計（emails）
  │   ├─ [2] 新着技術者上位（engineer_mail_sources, score>=70, 直近24h）
  │   ├─ [3] 新着案件上位（project_mail_sources, score>=70, 直近24h）
...
| 新着技術者 | `engineer_mail_sources` | `score`, `name`, `unit_price_max`, `skills`, `created_at` |
| 新着案件 | `project_mail_sources` | `score`, `title`, `customer_name`, `unit_price_max`, `required_skills`, `created_at` |
...
3. 当日の受信件数・新着SES・期限切れ契約が **すべて 0 件** ではないか（その場合「本日特筆すべき動きはありませんでした」のみ表示）
4. 管理者に依頼して `php artisan report:daily-sales --dry-run` でデータが取得できるか確認

### Q. AI 提案が表示されない
A. 要対応（新着SES＋期限切れ契約）が **0件** の場合、AI 提案は生成されません。
