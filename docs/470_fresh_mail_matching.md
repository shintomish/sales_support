...
## 10. 営業担当者向け 使い方ガイド（2026-05-19 拡張版）

### 10.1 機能の概要

開いている **1件の案件メール** または **1件の技術者メール** を起点に、**過去N日以内に受信した反対側のメール** から相性の良い候補をスコア順に並べる機能です。

従来の「登録済み技術者・登録済み案件」とのマッチングではなく、**まだ未登録の生メール同士をその場で突き合わせる** ことができます。

> **例:** 今朝届いた案件メールを開く → 過去3日に届いた技術者メールの中から条件に合う3名を即座に確認 → そのまま提案送信。

### 10.2 利用画面

| 画面 | 起点 | マッチ対象 |
|---|---|---|
| `/matching/[案件メールID]` | 案件メール | 過去N日の **技術者メール** (engineer_mail_sources) |
| `/engineer-mails/[技術者メールID]` | 技術者メール | 過去N日の **案件メール** (project_mail_sources) |

### 10.3 ヘッダーのセレクト3点セット

「📨 メール」モードを ON にすると、画面上部にセレクトが並びます。

| セレクト | 選択肢 | 役割 |
|---|---|---|
| **📨 メール** | クリックで ON/OFF | 鮮度マッチングモードに切替 |
| **過去N日** | 1日 / 3日 / 7日 / 14日 / 30日 | マッチ候補を探す **期間の窓** を指定（デフォルト 7 日） |
| **スコア閾値** | 高（70+）/ 中（60+）/ 低（50+） | マッチスコアの **下限** を指定（デフォルト 70） |

### 10.4 想定される使い分け

#### ▼ 期間（過去N日）の使い分け
- **1〜3日** … 鮮度重視。今日明日に動く可能性の高い候補だけを見たいとき
- **7〜14日** … 候補数を増やしたいとき。長めの商談・タイミング待ちにも対応
- **30日** … マイナースキル案件・ピンポイント条件で母数を最大化したいとき

#### ▼ スコア閾値の使い分け
- **高（70+）** … 自信を持って提案できる候補のみ。日次運用のデフォルト
- **中（60+）** … スコア70に届かないが惜しい候補も拾いたいとき
- **低（50+）** … 母数が少ない案件・技術者で、可能性のあるものは全部見たいとき

> **段階的に絞り込みを緩めるのがコツ:**
> 「過去3日 × 高」で空振り → 「過去7日 × 中」に広げる → さらに足りなければ「過去14日 × 低」

### 10.5 表示切替（カード／一覧）

候補リストの右上にある **⊞（カード）／ ≡（リスト）** ボタンで表示形式を切り替えられます。
2026-05-19 リリースで両画面（matching / engineer-mails）に統一搭載されました。

- **カード表示** … スキル・単価・抽出理由まで詳しく確認したいとき
- **一覧表示** … 多数の候補をざっと眺めて当たりを付けたいとき

### 10.6 提案メール送信フロー

候補のチェックボックスを ON → ヘッダーの「📤 提案メール送信」を押下するだけです。
未登録の EngineerMailSource / ProjectMailSource は提案送信と同時に Engineer / Deal に自動変換されます（重複検出キー: メールアドレス＋氏名＋所属）。

### 10.7 日次レポートからの導線

毎朝の日次レポート（08:30 配信）の **「📨 有効と思われるメールリスト」** セクションの各カードに「マッチング画面を開く →」リンクがあります。
リンクから直接 `/matching/[id]` / `/engineer-mails/[id]` に飛び、過去3日 × スコア70+ で抽出済みの上位3件をそのまま提案送信できます。

詳細は **docs/550_daily_report_flow.md** を参照してください。

### 10.8 技術詳細（システム担当向け）

| パラメータ | 値 | 定義箇所 |
|---|---|---|
| 期間デフォルト | 7 日 | `FreshMailMatchingService::freshEngineerMails($days = 7)` |
| スコア閾値デフォルト | 70 | `FreshMailMatchingService::RESULT_SCORE_FLOOR` |
| 抽出品質スコア下限 | 30 | `FreshMailMatchingService::QUALITY_FLOOR` |
| 1リクエストあたり処理上限 | 300 件 | `FreshMailMatchingService::HARD_LIMIT` |
| API バリデーション | `min_score: in:50,60,70` | `ProjectMailController::freshEngineerMails` / `EngineerMailController::freshProjectMails` |
| dedup キー (EMS) | email + name + affiliation | `dedupEmsBy3Key` |
| dedup キー (PMS) | from_address + subject + customer_name | `dedupPmsBy3Key` |
