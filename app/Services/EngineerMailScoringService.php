...
class EngineerMailScoringService
{
    ...
    public const ROLE_SKILLS = [
        ...
        // ロースキル・非開発ロール（2026-07-10 追加）
        'コールセンター', 'カスタマーサポート', 'テレオペ', '電話対応', '問い合わせ対応',
        '一般事務', 'OA事務', 'データ入力', 'データエントリ', 'オペレーター',
        '障害対応', '一次対応', 'ITサポート', 'テクニカルサポート', '導入支援', 'マニュアル作成',
    ];
    ...
    private function extractUnitPrice(string $text): array
    {
        ...
        // 【単金】/【単 金】も対象（HTML配信メールで多い）。値行から範囲/単一を抽出。
        if (preg_match('/【\s*単[　\s]*[価金]\s*】(?:[（(][^）)]*[）)])?[：:　\s]*(.*?)(?:\n|$)/u', $text, $m)) {
            $line = $m[1];
            if (preg_match('/(\d{2,3})[万Mm]?[〜～~\-－](\d{2,3})/u', $line, $r)) {
                return [(int) $r[1], (int) $r[2]];
            }
        ...
        // ラベル付き単一値（全角スペース対応）: 単　価：80万, 単金 110万, 単金（税抜）：70万, 単金額：85万円
        // 単価/単金 の後ろに「額」が付く「単金額」「単価額」も許容する（HTML配信メールで多い）。
        if (preg_match('/(?:単[　\s]*[価金](?:[　\s]*額)?|希望単価|想定単価|月単価|月額)(?:[（(][^）)]*[）)])?[：:\s　]*[〜～~]?(\d{2,3})[万Mm]/u', $text, $m)) {
            $val = (int) $m[1];
            return [$val, $val];
        }
    ...
    private function extractSkills(string $text): array
    {
        ...
        foreach (app(\App\Services\SkillDictionary::class)->surfaces() as $surface) {
            if ($this->skillFound($text, $surface)) {
                $found[] = $surface;
            }
        }
